...
APP_HOSTNAME=$( kubectl -n webapp get ing webapp -o json | jq -r '.spec.rules[0].host' )
APP_ENDPOINT=$( kubectl -n webapp get ing webapp -o json | jq -r '.status.loadBalancer.ingress[0].hostname' )

echo -e "Make alias Route53 record for:\n\tDomain: \t\t${APP_HOSTNAME}\n\tAssosiated with elb: \t${APP_ENDPOINT}"
